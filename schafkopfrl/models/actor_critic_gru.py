...
class ActorCriticNetworkGRU(nn.Module):
    ...
    def forward(self, state_vector, allowed_actions):
        ...
        x = torch.cat((x, torch.squeeze(hidden1)), -1)
        x = F.relu(self.fc2(x))
    ...
    def preprocess(self, game_state, player):
        ...
        - game_history: x * 16
            - course_of_game: x * (12 + 4) each played card in order plus the player that played it (zeros after each trick)
        ...
        for trick in range(len(game_state.course_of_game)):
            ...
            if not all_cards_processed:
                course_of_game_enc = np.vstack((course_of_game_enc, np.zeros((1, 16))))
