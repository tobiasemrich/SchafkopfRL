...
class TransformerHistoryEncoder(nn.Module):
    def __init__(self, num_actions, emb_size_actions, emb_size_player, transformer_dim, num_heads, num_layers, max_seq_len):
        super().__init__()
    ...
    def forward(self, action_history, lengths):
        ...
        action_emb = self.action_embed(action_ids)  # (B, T, E1)
        player_emb = self.player_embed(player_ids)  # (B, T, E2)
        input_seq = torch.cat([action_emb, player_emb], dim=-1)  # (B, T, E1+E2)

        x = self.input_proj(input_seq)  # (B, T, D)

        # Create attention mask (True = pad, False = valid)
        # Shape must be (B, T)
        pad_mask = torch.arange(T, device=device).expand(B, T) >= lengths.unsqueeze(1)

        transformer_out = self.transformer(x, src_key_padding_mask=pad_mask)  # (B, T, D)

        # Option A: Take last valid step
        last_idx = (lengths - 1).clamp(min=0).unsqueeze(1).unsqueeze(2).expand(-1, 1, self.output_dim).long()  # (B, 1, D)
        pooled = transformer_out.gather(dim=1, index=last_idx).squeeze(1)  # (B, D)
