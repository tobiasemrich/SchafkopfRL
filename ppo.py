...
class PPO:
    def __init__(self, policy, lr, betas, gamma, K_epochs, eps_clip, batch_size, c1=0.5, c2=0.01):
        ...
        #self.policy_old = ActorCriticNetwork().to(device)
    ...
    def update(self, memory, i_episode):
        ...
        #old_states = []
        #transposed_states = list(map(list, zip(*memory.states)))
        #old_states.append(torch.stack(transposed_states[0]).to(device).detach())

        #sequences = [torch.squeeze(seq[0], dim=1).to(device).detach() for seq in transposed_states[1]]
        #old_states.append(sequences)

        #old_actions = torch.stack(memory.actions).to(device).detach()
        #old_allowed_actions = torch.stack(memory.allowed_actions).to(device).detach()
        ...
        self.policy_old.load_state_dict(self.policy.state_dict())
        self.writer.add_scalar('Loss/train', avg_loss/count, i_episode)
...
def evaluate_single(checkpoint_folder, model_class, eval_file, runs, random_policy):
    generations = [int(f[:6]) for f in listdir(checkpoint_folder) if f.endswith(".pt")]
    if len(generations) > 0:
        ...
        gs = Game_Simulation(policy, random_policy, random_policy, random_policy, 1)
        all_rewards = np.array([0, 0, 0, 0])
        for j in range(runs):
            game_state = gs.run_simulation()
            rewards = np.array(game_state.get_rewards())
        ...
        f.write(str(max_gen) + " : " + str(all_rewards[0]))
        f.write("\n")
        f.close
...
def main():
    ...
    ############## Hyperparameters ##############
    log_interval = 2000  # print some logs in this interval
    max_episodes = 900000  # max training episodes


    update_timestep = 2000  # update policy every n games #<------------------------------------change to 100
    evaluate_timestep = 10000 #save checkpoints every n
    ...
    for i_episode in range(max_gen+1, max_episodes + 1):
        ...
        game_state = gs.run_simulation()
        t1 = time.time_ns()
        #running_reward += game_state.get_rewards()[0]
        ...
        if i_episode % log_interval == 0:
            ...
            ppo.writer.add_scalar('Games/weiter', game_count[0]/log_interval, i_episode)
            ppo.writer.add_scalar('Games/sauspiel', game_count[1] / log_interval, i_episode)
            ppo.writer.add_scalar('Games/wenz', game_count[2] / log_interval, i_episode)
            ppo.writer.add_scalar('Games/solo', game_count[3] / log_interval, i_episode)
        ...
        if i_episode % evaluate_timestep == 0:
            ...
            #current_policy = ActorCriticNetwork()
            #current_policy.to(device)
            #current_policy.load_state_dict(ppo.policy_old.state_dict())
            #evaluate_single(checkpoint_folder, "eval.txt", 400,random_policy)
