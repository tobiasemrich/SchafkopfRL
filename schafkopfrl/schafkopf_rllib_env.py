...
class SchafkopfEnv(MultiAgentEnv):
    ...
    def reset(self, seed):
        ...
        return {"0": self._compile_state()}, None

    def step(self, action_dict: Dict[Any, Any]):

        action = next(iter(action_dict.values()))
        if len(action_dict) != 1 or action not in self.last_allowed_actions:
        ...
        return self._compile_state(), {str(i): rewards[i] for i in range(4)}, {str(i): terminal for i in range(4)}
    ...
    def render(self):
        ...
        for i in range(4):
            ...
            if self.public_gamestate.bidding_round[i] is None:
                ...
                if self.public_gamestate.bidding_round[i][1] != None:
                    ...
                    br += "weiter! "
                br += "[{:0.3f}]  ".format(self.public_gamestate.action_probabilities[0][i])
        print(br + "\n")
        ...
        for p in range(4):
            if self.public_gamestate.contra[p]:
                ...
                contra_str += "[{:0.3f}]".format(self.public_gamestate.action_probabilities[1][p])
                contra_str += "  |   "
        for p in range(4):
            if self.public_gamestate.retour[p]:
                contra_str += "player " + str(p)
                contra_str += "[{:0.3f}]".format(self.public_gamestate.action_probabilities[2][p])
                contra_str += "  |   "
        print(contra_str + "\n")
            ...
            for trick in range(8):
                ...
                for player in range(4):
                    ...
                    if self.public_gamestate.course_of_game_playerwise[trick][player] == [None, None]:
                        ...
                        trick_str_ += "[{:0.3f}]".format(self.public_gamestate.action_probabilities[trick + 3][player])
                        if self.public_gamestate.course_of_game_playerwise[trick][player] in self.rules.get_sorted_trumps(
                                self.public_gamestate.game_type):
                            trick_str_ += '\033[0m'
